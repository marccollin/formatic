...
public class EditorService {
    ...
    private static List<Editor2> getHardcodedEditors() {
        Editor2 editor1 = new Editor2();
    ...
    public Optional<Editor2> getEditorById(Long id) {
        List<Editor2> editors = getHardcodedEditors();

        // Parcourt la liste pour trouver l'éditeur correspondant à l'ID
        for (Editor2 editor : editors) {
            if (editor.getIdEditeur() != null && editor.getIdEditeur().equals(id)) {
                return Optional.of(editor); // Retourne l'éditeur trouvé dans un Optional
            }
        }
